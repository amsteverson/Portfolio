...
FILENAME MYLOG "/folders/myfolders/MIS 500 Portfolio Project/Exercise3B_log.TXT";
FILENAME MYPRINT "/folders/myfolders/MIS 500 Portfolio Project/Exercise3B_OUTPUT.TXT";
...
MERGE DIAB          (IN=AA KEEP=DUPERSID CONDIDX CCCODEX)
      MEPS.H178IF1 (IN=BB KEEP=CONDIDX  EVNTIDX );
   BY CONDIDX;
      IF AA AND BB ;
RUN;
...
PROC SORT DATA=MEPS.H178A  OUT=PMED (KEEP=LINKIDX RXXP15X  RXSF15X--RXOU15X RENAME=(LINKIDX=EVNTIDX));
  BY LINKIDX;
RUN;
...
DATA  FY1;
MERGE MEPS.H181 (IN=AA KEEP=DUPERSID VARPSU VARSTR PERWT15F /*ADD MORE VARIABLES*/)
